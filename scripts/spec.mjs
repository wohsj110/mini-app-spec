#!/usr/bin/env node
// spec.mjs — the sole transaction layer of mini-app-spec (implements references/contract.md v1)
// Commands: new / extract / save / status / validate / refresh-sources / recovery
// record-run and --retemplate belong to D6-7; this file reserves error placeholders for them.
...
// canonical serialization (contract.md §7.1): NFC, keys sorted, array order preserved, CRLF→LF, shortest numbers
...
export function parseArtifact(html) {
  ...
  for (const id of BLOCK_IDS) {
    ...
    if (hits.length === 0) throw new Error(`missing data block ${id}`);
    if (hits.length > 1) throw new Error(`STR-05: data block ${id} appears ${hits.length} times`);
    blocks[id] = JSON.parse(hits[0][1]);
  }
  const templates = {};
  for (const m of html.matchAll(/<template id="(ui-[^"]+)">([\s\S]*?)<\/template>/g)) {
    if (templates[m[1]] !== undefined) throw new Error(`STR-05: duplicate template ${m[1]}`);
    templates[m[1]] = m[2];
  }
...
const STUB_ENGINE_JS = `document.getElementById('app').textContent = 'mini-app-spec: engine view lands in D6-7; contract data lives in the page-source data blocks.';`;
...
export function validateData(data, opts = {}) {
  ...
  if ((data.specVersion ?? 1) > 1) return [F('STR-06', 'specVersion', `specVersion ${data.specVersion} exceeds supported version ⇒ blocked`)];

  // ── STR-02 required fields (minimal schema)
  const req = (obj, fields, ref) => { for (const f of fields) if (obj?.[f] === undefined) out.push(F('STR-02', ref, `missing required field ${f}`)); };
  req(c?.meta, ['id', 'title'], 'meta');
  for (const k of ['sources', 'flows', 'states', 'transitions', 'requirements', 'scenarios', 'issues']) if (!Array.isArray(c?.[k])) out.push(F('STR-02', k, `contract.${k} must be an array`));
  ...
  for (const s of c.states) {
    ...
    if (s.figma === undefined && s.figmaNote === undefined) out.push(F('STR-02', s.id, 'exactly one of figma|figmaNote is required'));
  }
  ...
  for (const r of l.runs ?? []) {
    ...
    if (r.scenarioFingerprint === undefined || r.verificationHash === undefined) out.push(F('LED-02', r.id, 'run missing scenarioFingerprint/verificationHash'));
    if (r.dirty === true && r.dirtyDiffHash === undefined) out.push(F('LED-01', r.id, 'dirty run missing dirtyDiffHash'));
  }
  ...
  // ── STR-03/04 ID format and uniqueness
  const ids = new Map();
  const collect = (arr, kind) => { for (const x of arr ?? []) { if (x.id !== undefined) { if (!ID_RE.test(x.id)) out.push(F('STR-03', x.id, `invalid ID format (${kind})`)); if (ids.has(x.id)) out.push(F('STR-04', x.id, `duplicate ID (${kind} vs ${ids.get(x.id)})`)); else ids.set(x.id, kind); } } };
  ...
  for (const s of c.scenarios) for (const ex of s.examples ?? []) { if (ids.has(ex.id)) out.push(F('STR-04', ex.id, 'duplicate example ID')); else ids.set(ex.id, 'example'); }

  // ── ALG-01 derived status must not be persisted
  const forbid = (arr, keys, kind) => { for (const x of arr ?? []) for (const k of keys) if (x[k] !== undefined) out.push(F('ALG-01', x.id, `${kind} must not persist derived field ${k}`)); };
  ...
  if (c.meta.status !== undefined || c.meta.acceptedNote !== undefined) out.push(F('ALG-01', 'meta', 'meta must not carry status fields'));

  // ── REF-01 references must resolve
  const has = (id) => ids.has(id);
  const refCheck = (id, ref, what) => { if (ref != null && !has(ref)) out.push(F('REF-01', id, `${what} reference unresolvable: ${ref}`)); };
  ...
  for (const r of l.runs ?? []) {
    ...
    for (const er of r.exampleResults ?? []) {
      ...
      if (er.exampleRef === `${r.scenarioRef}-implicit`) continue; // implicit example for scenarios without Examples (contract.md §4)
  ...
  for (const f0 of c.flows) {
    ...
    for (const t of trans) {
      ...
      if (fromS && fromS.flowRef !== t.flowRef) out.push(F('FSM-06', t.id, 'from state does not belong to this flow'));
      if (toS && toS.flowRef !== t.flowRef) out.push(F('FSM-06', t.id, 'to state does not belong to this flow'));
    }
    const entries = states.filter((s) => s.entry && s.entry !== 'none');
    if (states.length && entries.length === 0) out.push(F('FSM-01', f0.id, 'flow has no entry state'));
    // FSM-02 reachability (any edge)
    ...
    for (const s of states) if (!seen.has(s.id) && entries.length) out.push(F('FSM-02', s.id, 'state unreachable'));
    // FSM-04 multiple guardless out-edges require exactly one default
    for (const s of states) {
      const guardless = trans.filter((t) => t.from === s.id && (t.guard == null));
      if (guardless.length > 1 && guardless.filter((t) => t.isDefault).length !== 1) out.push(F('FSM-04', s.id, `${guardless.length} guardless out-edges but default count ≠1`));
    }
    // FSM-03/05 default path: a single out-edge counts as default
    ...
    for (const e of entries) {
      ...
      if (loopEdge) { if (!loopEdge.loopAllowed) out.push(F('FSM-05', loopEdge.id, 'default path loops without loopAllowed')); if (!reached) out.push(F('FSM-03', e.id, 'default path trapped in a loop, never reaches terminal')); }
      else if (!reached) out.push(F('FSM-03', e.id, 'default path does not reach terminal'));
    }
  }

  // ── ALG-02/03 requirement coverage and waivers
  ...
  for (const r of c.requirements) {
    ...
    if (!covered) {
      if (!r.waiver) out.push(F('ALG-02', r.id, 'behavior requirement has no scenario and no waiver'));
      else if (!waiverOk(r.waiver)) out.push(F('ALG-03', r.id, 'waiver missing reason/authority/approvedAt'));
    }
  }

  // ── run layer: ALG-04/05, LED-01
  const scnOf = (id) => c.scenarios.find((s) => s.id === id);
  for (const r of l.runs ?? []) {
    const scn = scnOf(r.scenarioRef);
    if (r.result === 'passed' && (!Array.isArray(r.evidence) || r.evidence.length === 0)) out.push(F('ALG-04', r.id, 'passed run has empty evidence'));
    // strict evidence-file checks apply only to "applicable" runs (fingerprint+vh match the current scenario); historical runs are display-only, excluded from derivation, and their evidence may legitimately drift as the artifact evolves
    ...
    if (opts.strict && r.result === 'passed' && rApplicable) for (const ev of r.evidence ?? []) {
      ...
      if (!fs.existsSync(p)) out.push(F('ALG-04', r.id, `evidence file missing: ${ev.path}`));
      else if (ev.sha256 && sha256hex(fs.readFileSync(p)) !== ev.sha256) out.push(F('ALG-04', r.id, `evidence hash mismatch: ${ev.path}`));
    }
    if (scn?.verification?.kind === 'assert' && r.result === 'passed' && !(r.testCount > 0)) out.push(F('ALG-05', r.id, 'assert run marked passed but testCount missing or 0'));
    if (scn?.verification?.kind === 'db' && (r.buildVariant === undefined || r.devicePackage === undefined || r.deviceSerial === undefined)) out.push(F('LED-01', r.id, 'db/device scenario run missing device context'));
  }

  // ── ACC-*: acceptance "preconditions at write time" (contract.md §6.6).
  // ACC-01/02 reconstruct the batch current at acceptance write time from changelog order (holds for historical acceptances too);
  // ACC-03/04/05/06 depend on current contract state and are evaluated only when the latest write is the acceptance itself —
  // legitimate appends after an acceptance (new batch/issue/annotation) never retroactively taint it; their impact shows on the five-tuple display axis.
  ...
  for (const acc of l.acceptances ?? []) {
    ...
    if (acc.sourceBatchId !== batchAtAcc) out.push(F('ACC-01', acc.id, `acceptance references batch ${acc.sourceBatchId}, not ${batchAtAcc}, the latest batch at its write time`));
    ...
    for (const src of c.sources) {
      ...
      const bad = !ob ? 'unknown (not observed in this batch)' : ob.outcome === 'unavailable' ? 'unavailable' : (ob.outcome === 'changed' && !ob.decidedImpact) ? 'changed but not adjudicated' : null;
      if (bad && !waived.has(src.id)) out.push(F('ACC-02', acc.id, `source ${src.id} ${bad} and no sourceWaiver`));
    }
    if (acc.revision >= maxRev) {
      for (const f0 of c.flows) if (f0.core && f0.alignStatus !== 'aligned') out.push(F('ACC-06', acc.id, `core flow ${f0.id} not aligned`));
      for (const i of c.issues) if (i.type === 'blocker' && i.status === 'open') out.push(F('ACC-03', acc.id, `open blocker exists: ${i.id}`));
      for (const an of l.annotations ?? []) if (an.status === 'proposed') out.push(F('ACC-04', acc.id, `proposed annotation exists: ${an.id}`));
      for (const [scnId, v] of Object.entries(derived.scenarioValue)) {
        ...
        if (flow?.core && v !== 'passed' && !(acc.deviationsAcknowledged ?? []).some((d) => d.ref === scnId && waiverOk(d.waiver))) out.push(F('ACC-05', acc.id, `core scenario ${scnId}=${v} without deviation waiver`));
      }
    }
  }
  return out;
...
// ───────────────────────── derived status (contract.md §6, computed, never stored) ─────────────────────────
...
export function deriveStatus(data) {
  ...
  for (const scn of c.scenarios ?? []) {
    ...
    // ordering semantics [N-7]: highest revision wins; within the same revision, later array position wins
...
// ───────────────────────── template safety (contract.md §8.1) ─────────────────────────
...
export function validateTemplates(templates, contract) {
  ...
  for (const [tplId, htmlIn] of Object.entries(templates)) {
    ...
    for (const m of htmlIn.matchAll(/<\s*\/?\s*([a-zA-Z][a-zA-Z0-9-]*)((?:[^>"']|"[^"]*"|'[^']*')*)>/g)) {
      ...
      if (!TPL_TAGS.has(tag)) { out.push(F('TPL-01', tplId, `forbidden tag <${tag}>`)); continue; }
      const attrs = [...m[2].matchAll(/([a-zA-Z-]+)\s*=\s*("([^"]*)"|'([^']*)')/g)].map((a) => [a[1].toLowerCase(), a[3] ?? a[4] ?? '']);
      for (const [name, val] of attrs) {
        if (name.startsWith('on')) { out.push(F('TPL-01', tplId, `forbidden event attribute ${name}`)); continue; }
        if (!TPL_ATTRS.has(name)) { out.push(F('TPL-01', tplId, `forbidden attribute ${name}`)); continue; }
        if (name === 'style' && /url\s*\(/i.test(val)) out.push(F('TPL-01', tplId, 'url() forbidden inside style'));
        if (/^(javascript|vbscript):/i.test(val.trim())) out.push(F('TPL-01', tplId, `forbidden protocol in ${name}`));
        if (tag === 'input' && name === 'type' && !INPUT_TYPES.has(val)) out.push(F('TPL-01', tplId, `input type=${val} not in allowlist`));
        if (tag === 'img' && name === 'src') {
          const dm = val.match(/^data:(image\/(png|jpeg|webp));base64,(.*)$/);
          if (!dm) out.push(F('TPL-03', tplId, 'img allows only data:image/(png|jpeg|webp)'));
          else if (Buffer.from(dm[3], 'base64').length > 64 * 1024) out.push(F('TPL-03', tplId, 'img data URI exceeds 64KB'));
        }
        if ((name === 'data-show-in' || name === 'data-on-in') && contract) {
          for (const tok of val.split(/[\s,]+/).filter(Boolean)) if (!(contract.states ?? []).some((st) => st.id === tok)) out.push(F('TPL-02', tplId, `${name} references nonexistent state: ${tok}`));
        }
        if (name === 'data-go' && contract) {
          const tr = (contract.transitions ?? []).find((t) => t.id === val);
          // one flow, one screen: template ownership = every state whose uiTemplateRef points at this template (shared screen); with no referrer, fall back to the ui-<stateId> naming
          const owners = (contract.states ?? []).filter((st) => st.uiTemplateRef === tplId).map((st) => st.id);
          if (!owners.length) owners.push(stateId);
          if (!tr) out.push(F('TPL-02', tplId, `data-go points to nonexistent transition: ${val}`));
          else if (!owners.includes(tr.from)) out.push(F('TPL-02', tplId, `data-go=${val}: from(${tr.from}) is not in the state set sharing this template [${owners.join(',')}]`));
        }
      }
    }
  }
...
export function validateEnvelope(a) {
  ...
  for (const k of Object.keys(h)) if (a.stamp?.hashes?.[k] !== h[k]) out.push(F('ENV-01', k, `${k} hash mismatch (out-of-band edit or bypassed save)`));
  // ENV-02: changelog+recoveries chain
  const revs = [...(a.ledger.changelog ?? []).map((e) => e.rev)];
  const uniq = new Set(revs);
  if (uniq.size !== revs.length) out.push(F('ENV-02', 'changelog', 'duplicate changelog revision'));
  if (a.stamp && revs.length && Math.max(...revs) !== a.stamp.revision) out.push(F('ENV-02', 'stamp', `stamp.revision(${a.stamp.revision}) ≠ max changelog rev(${Math.max(...revs)})`));
  return out;
}
...
export function validateAgainstGit(filePath, a) {
  ...
  try {
    ...
    if (!gate) { out.push(F('GIT-02', 'anchor', 'no gate commit anchor (unavailable, not a pass)', 'info')); return out; }
    ...
    if (a.stamp.revision < old.stamp.revision) out.push(F('GIT-01', 'revision', `current r${a.stamp.revision} < gate r${old.stamp.revision}: suspected replay/out-of-band overwrite`));
    else if (a.stamp.revision === old.stamp.revision) {
      const same = canonical(computeHashes(a)) === canonical(old.stamp.hashes);
      if (!same) out.push(F('GIT-01', 'revision', 'same revision as gate but different content: out-of-band overwrite'));
    } else {
      const revs = new Set((a.ledger.changelog ?? []).map((e) => e.rev));
      for (let r = old.stamp.revision + 1; r <= a.stamp.revision; r++) if (!revs.has(r)) out.push(F('GIT-01', 'chain', `revision chain broken: missing changelog for r${r}`));
    }
  } catch (e) { out.push(F('GIT-02', 'anchor', `git anchor unavailable: ${String(e.message).split('\n')[0]} (unavailable, not a pass)`, 'info')); }
  return out;
}
...
// ───────────────────────── transactional write (contract.md §9) ─────────────────────────
...
export function txWrite(filePath, mutate, { expectRevision } = {}) {
  ...
  let fd;
  try { fd = fs.openSync(lockPath, 'wx'); } catch { throw new Error(`concurrency conflict: lockfile already exists (${lockPath})`); }
  fs.writeSync(fd, JSON.stringify({ pid: process.pid, startedAt: Date.now() })); fs.closeSync(fd);
  const bakPath = filePath + '.bak';
  try {
    const orig = fs.readFileSync(filePath, 'utf8');
    const a = parseArtifact(orig);
    if (expectRevision !== undefined && a.stamp.revision !== expectRevision) throw new Error(`CAS conflict: file revision=${a.stamp.revision}, expected ${expectRevision}`);
    const next = mutate(a); // mutate returns the new artifact object (with the updated stamp)
    ...
    if (errs.length) throw new Error('validator rejected the write:\n' + errs.map((e) => `  [${e.code}] ${e.ref}: ${e.message}`).join('\n'));
    const content = buildArtifact(next);
    parseArtifact(content); // read-back self-check before rename
    ...
    try { parseArtifact(fs.readFileSync(filePath, 'utf8')); } catch (e) { fs.copyFileSync(bakPath, filePath); throw new Error(`read-back after rename failed; rolled back from .bak: ${e.message}`); }
...
function cmdNew(filePath, { id, title }) {
  if (fs.existsSync(filePath)) throw new Error(`already exists: ${filePath} (new never overwrites)`);
  if (!id || !title) throw new Error('Usage: spec.mjs new <path> --id FEAT-x --title <title>');
  const a = {
    specVersion: 1,
    ...
    progress: { stage: 'intake', currentFlow: null, nextAction: 'register sources and build the flow map', pendingDecisions: [], worthIt: null, updatedAt: new Date().toISOString() },
  ...
  console.log(`created ${filePath} (r1)`);
}
...
function cmdExtract(filePath, { out }) {
  ...
  if (out) { fs.writeFileSync(out, payload); console.log(`exported ${out}`); } else console.log(payload);
}
...
function cmdSave(filePath, { data, expectRevision, gate, retemplate }) {
  if (!data && !retemplate) throw new Error('Usage: spec.mjs save <path> [--data <payload.json>] [--retemplate] [--expect-revision N] [--gate <name>]');
  ...
  console.log(`saved ${filePath} (r${next.stamp.revision})`);
  if (gate) {
    const root = gitRoot(filePath), rel = path.relative(root, path.resolve(filePath));
    execFileSync('git', ['add', '--', rel], { cwd: root });
    // Note: no `--` pathspec separator — the environment's git wrapper appends a trailer -m at the end of the args,
    // and anything after `--` gets treated as a pathspec and blows up (known pitfall). rel never starts with -, so pass it directly.
    execFileSync('git', ['commit', '-m', `spec(${next.contract.meta.id}): gate ${gate} r${next.stamp.revision}`, '--only', rel], { cwd: root });
    console.log(`gate commit done: gate ${gate} r${next.stamp.revision}`);
  }
}
  ...
  const next = txWrite(filePath, (a) => {
    ...
    a.contract.sources.forEach((src, i) => {
      ...
      if ((src.kind === 'doc' || src.kind === 'code') && fs.existsSync(p) && fs.statSync(p).isFile()) {
        ...
        obs.unavailableReason = `file-source adapter cannot fetch (kind=${src.kind}; not a local file or missing)`;
      }
      a.ledger.sourceObservations.push(obs);
    });
    return bumpRevision(a, 'source', [{ ref: batchId, fields: [] }]);
  });
  console.log(`refresh-sources done (r${next.stamp.revision})`);
...
function cmdStatus(filePath) {
  ...
  lines.push(`Five-tuple: specAcceptance=${d.specAcceptance} | implementation=${d.implementationStatus} | sources={${Object.entries(d.sourceStatus).map(([k, v]) => `${k}:${v}`).join(', ')}} | openBlockers=${d.openBlockers} | proposedAnnotations=${d.proposedAnnotations}`);
  ...
  lines.push(`changelog(last ${recent.length}): ` + recent.map((e) => `r${e.rev}:${e.type}`).join(' '));
...
function cmdRecovery(filePath, { reason }) {
  if (!reason) throw new Error('Usage: spec.mjs recovery <path> --reason <reason>');
  const root = gitRoot(filePath), rel = path.relative(root, path.resolve(filePath));
  ...
  if (!gate) throw new Error('recovery failed: no trusted gate commit baseline');
  const [hash] = gate.split(' ');
  ...
  next.ledger.recoveries.push({ at: new Date().toISOString(), reason, fromRevision, trustedBaseCommit: hash, diffSummary: `restored to gate r${baseRev}` });
  ...
  console.log(`recovery done: baseline gate r${baseRev} (${hash.slice(0, 7)}) → r${next.stamp.revision}`);
}

// ───────────────────────── verification gate / record-run / accept (contract.md §7) ─────────────────────────

function cmdConfirmCommand(filePath, { scenario, command }) {
  if (!scenario || !command) throw new Error('Usage: spec.mjs confirm-command <path> --scenario SCN-x --command "…"');
  const next = txWrite(filePath, (a) => {
    const scn = a.contract.scenarios.find((s) => s.id === scenario);
    if (!scn) throw new Error(`scenario not found: ${scenario}`);
  ...
  console.log(`verification gate: ${scenario} command frozen (r${next.stamp.revision})`);
}
...
function judgeDb(assertion, stdout) {
  ...
  if (Number.isNaN(actual)) return { result: 'blocked', note: `last output line is not numeric: ${lines[lines.length - 1] ?? '(empty)'}` };
  const ok = evalPredicate(assertion.predicate, actual);
  if (ok === null) return { result: 'blocked', note: `unparsable predicate: ${assertion.predicate}` };
  return { result: ok ? 'passed' : 'failed', note: `db: actual=${actual} predicate=${assertion.predicate}` };
}

// file-tree/package existence reconciliation (the "file-tree reconciliation" from the original mini-app specs)
function judgeFile(root, assertion) {
  const paths = assertion.paths ?? [];
  if (!paths.length) return { result: 'blocked', note: 'assertion.paths is empty' };
  ...
  return { result: missing.length === 0 ? 'passed' : 'failed', evidence, note: `file: ${paths.length - missing.length}/${paths.length} present${missing.length ? '; missing: ' + missing.join(', ') : ''}` };
}

// template/config/secret binding reconciliation (verifies existence and counts only, never stores values — secret-safe)
function judgeGrep(root, assertion) {
  if (!assertion.pattern || !assertion.path) return { result: 'blocked', note: 'assertion requires {path, pattern, predicate}' };
  let re; try { re = new RegExp(assertion.pattern, 'g'); } catch (e) { return { result: 'blocked', note: `unparsable pattern: ${e.message}` }; }
  ...
  if (ok === null) return { result: 'blocked', note: `unparsable predicate: ${assertion.predicate}` };
  return { result: ok ? 'passed' : 'failed', evidence: hitFiles.slice(0, 20).map((f0) => ({ path: f0, sha256: sha256hex(fs.readFileSync(path.resolve(root, f0))), type: 'grep-hit', size: 0, redacted: true })), note: `grep: ${count} hits (files=${files.length}) predicate=${assertion.predicate} (count only, matched values never stored)` };
}
...
function cmdRecordRun(filePath, opts) {
  const { scenario } = opts;
  if (!scenario) throw new Error('Usage: spec.mjs record-run <path> --scenario SCN-x [--timeout ms] [--build-variant v --device-package p --device-serial s]');
  ...
  if (!['assert', 'db', 'file', 'grep'].includes(v.kind)) throw new Error(`judge ${v.kind} not implemented yet (assert/db/file/grep supported; mock/manual take their own paths)`);
  const needsCmd = ['assert', 'db'].includes(v.kind);
  if (needsCmd && (!v.command || !v.commandConfirmedAt)) throw new Error(`command not frozen via the verification gate; run confirm-command first`);
  if (v.kind === 'db' && !(opts.buildVariant && opts.devicePackage && opts.deviceSerial)) throw new Error('db scenarios require --build-variant/--device-package/--device-serial (LED-01)');
  const root = gitRoot(filePath);
  ...
  // the script executes the frozen command itself (the agent supplies the command, never the result); file/grep are judge-only and reconcile against the repo's current state directly
  ...
  let judge;
  if (sp.error && !timedOut) judge = { result: 'blocked', note: `execution failed: ${sp.error.message}` };
  ...
  // evidence: output log stored in evidence/ next to the artifact
  ...
  console.log(`${runId}: ${judge.result} (${judge.note ?? ''}) → ${scenario} derived status=${d.scenarioValue[scenario]} (r${next.stamp.revision})`);
}

function cmdAccept(filePath, { verbatim }) {
  if (!verbatim) throw new Error('Usage: spec.mjs accept <path> --verbatim "user\'s verbatim words" (refresh-sources required first)');
  const next = txWrite(filePath, (a) => {
    const batches = []; for (const o of a.ledger.sourceObservations) if (!batches.includes(o.batchId)) batches.push(o.batchId);
    const latest = batches[batches.length - 1];
    if (!latest) throw new Error('no refresh batch exists: refresh-sources is required before acceptance');
  ...
  console.log(`acceptance recorded (r${next.stamp.revision}): ${verbatim}`);
}

function cmdMergeFeedback(filePath, { data }) {
  if (!data) throw new Error('Usage: spec.mjs merge-feedback <path> --data <JSON file exported from the browser>');
  const payload = JSON.parse(fs.readFileSync(data, 'utf8'));
  if (!Array.isArray(payload.annotations) || !payload.annotations.length) throw new Error('payload has no annotations');
  const next = txWrite(filePath, (a) => {
    if (payload.specId && payload.specId !== a.contract.meta.id) throw new Error(`payload specId(${payload.specId}) ≠ contract(${a.contract.meta.id})`);
    if (payload.revision != null && payload.revision < a.stamp.revision) console.warn(`warning: annotations were exported from r${payload.revision}, contract is now at r${a.stamp.revision} — stale anchors get blocked by REF-01; manually re-check context for the rest`);
    ...
    return a; // targetId resolvability is gated by validator REF-01
  });
  console.log(`merge-feedback: merged ${payload.annotations.length} item(s) as proposed (r${next.stamp.revision}); each must be adjudicated (resolved/rejected) before acceptance`);
}

// ── progress: progress/self-assessment writes (the progress block stays outside the envelope; no revision bump, no changelog entry)
function cmdProgress(filePath, opts) {
  ...
  if (!Object.keys(fields).length) throw new Error('Usage: spec.mjs progress <path> [--stage s] [--next <text>] [--worth-it <text>] [--current-flow FLOW-x|null]');
  const next = txWrite(filePath, (a) => { a.progress = { ...(a.progress || {}), ...fields, updatedAt: new Date().toISOString() }; return a; });
  console.log(`progress updated (revision stays r${next.stamp.revision} — progress is an untrusted block, outside envelope/changelog)`);
}

// ── review: local review session (borrows lavish-axi's AXI shape: foreground blocking long-poll, feedback queue persisted so nothing is ever lost, Send & End closes the session)
// zero byte changes, zero injection into the artifact: the annotation UI ships with the engine (click-to-anchor → localStorage staging); the wrapper page shares localStorage with the same-origin artifact,
// and the wrapper (with its own relaxed CSP) does the POST on its behalf — the artifact's own default-src 'none' stays untouched.
export function startReviewServer(filePath, { port = 0 } = {}) {
  ...
  const srv = http.createServer((req, res) => {
    ...
    if (u.pathname === '/artifact') { res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' }); res.end(fs.readFileSync(abs)); return; } // re-read from disk and forwarded verbatim on every request, zero rewrites
...
async function cmdReview(filePath, opts) {
  ...
  if (pending.length) console.log(`note: unconsumed feedback queue already in this directory: ${pending.join(', ')} (the queue is never lost — merge-feedback can process it first)`);
  const s = await startReviewServer(filePath, { port: Number(opts.port ?? 0) });
  const url = `http://127.0.0.1:${s.port}/`;
  console.log(`review: ${url} (r${s.revision}, artifact byte-identical, loopback address only)\nWaiting for the user to submit annotations in the browser… if killed or timed out, just rerun; submitted queues are never lost.`);
  if (!opts.noOpen && process.platform === 'darwin') { try { execFileSync('open', [url]); } catch (_) { } }
  const timer = setTimeout(() => { console.log(`review: no submission within ${timeoutMs / 1000}s, exiting this run (rerun to keep waiting; browser-local drafts are unaffected)`); process.exit(2); }, timeoutMs);
  const sub = await s.waitForSubmission();
  clearTimeout(timer); await s.close();
  if (sub.count) console.log(`received ${sub.count} annotation(s) → ${sub.feedbackPath}\nNext: node spec.mjs merge-feedback ${filePath} --data ${sub.feedbackPath}`);
  if (sub.end) console.log('user chose Send & End: session over; do not reopen the review uninvited.');
  else if (!sub.count) console.log('received an empty submission (no annotations).');
  process.exit(0); // the browser may hold keep-alive connections; exit explicitly so the foreground blocking call returns cleanly
}
...
function cmdExportMd(filePath, { out }) {
  ...
  L2.push(`> Downgraded export from mini-app-spec.html (portable, read-only; the authoritative artifact is the HTML+contract).`, '');
  L2.push(`**Status**: specAcceptance=${d.specAcceptance} · implementation=${d.implementationStatus} · openBlockers=${d.openBlockers} · proposedAnnotations=${d.proposedAnnotations}`, '');
  L2.push('## Sources', '');
  for (const s of c.sources) L2.push(`- \`${s.id}\` ${s.kind} · ${s.locator} (${s.version}) — ${s.role} [${d.sourceStatus[s.id]}]`);
  for (const f0 of c.flows) {
    L2.push('', `## ${f0.title} (\`${f0.id}\`${f0.core ? ' · core' : ''} · ${f0.risk === 'high' ? 'high-uncertainty' : 'low-risk'} · ${f0.alignStatus})`, '');
    L2.push('States: ' + f0.stateRefs.map((sid) => { const st = c.states.find((x) => x.id === sid); return st ? `\`${st.id}\`${st.name}${st.terminal ? '(terminal)' : ''}` : sid; }).join(' · '));
    L2.push('', '| Transition | From→To | Trigger | Result | default/guard |', '|---|---|---|---|---|');
    ...
    for (const s of c.scenarios.filter((x) => x.flowRef === f0.id)) {
      ...
      for (const g of s.given) L2.push(`- Given ${g}`);
      L2.push(`- When ${s.when}`);
      for (const t of s.then) L2.push(`- Then ${t}`);
      for (const ex of s.examples ?? []) L2.push(`- Example \`${ex.id}\`: ${JSON.stringify(ex.values)}`);
    }
  }
  if (c.issues.length) { L2.push('', '## Open issues', ''); for (const i of c.issues.filter((x) => x.status === 'open')) L2.push(`- \`${i.id}\` (${i.type}) ${i.question}`); }
  if ((a.ledger.runs ?? []).length) { L2.push('', '## Run log', ''); for (const r of a.ledger.runs) L2.push(`- \`${r.id}\` → ${r.scenarioRef}: ${r.result} (${r.at}, evidence×${(r.evidence ?? []).length})`); }
  const md = L2.join('\n') + '\n';
  const dest = out ?? filePath.replace(/\.html$/, '.md');
  fs.writeFileSync(dest, md);
  console.log(`export-md: ${dest} (${md.length} bytes, read-only downgraded artifact, outside the envelope)`);
}

function cmdAnnotate(filePath, { id, status }) {
  if (!id || !['resolved', 'rejected', 'proposed', 'outdated'].includes(status)) throw new Error('Usage: spec.mjs annotate <path> --id ANN-x --status resolved|rejected|proposed|outdated (the user adjudicates; the agent only records)');
  const next = txWrite(filePath, (a) => {
    const an = a.ledger.annotations.find((x) => x.id === id);
    if (!an) throw new Error(`annotation not found: ${id}`);
    an.status = status;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    ...
    else { console.log('Usage: spec.mjs <new|extract|save|status|validate|refresh-sources|recovery|confirm-command|record-run|accept|review> <path> [options]'); process.exitCode = 2; }
